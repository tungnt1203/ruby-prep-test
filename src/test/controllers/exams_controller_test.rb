...
class ExamsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "get index without login redirects to login and stores return_to" do
    ...
    assert_equal "/exams?exam_code=#{exam_session.hash_id}&room_code=#{exam_room.room_code}", session[:return_to]
  ...
  test "post create without login redirects to login" do
    ...
    delete logout_path
    post exams_path, params: { exam_code: exam_session.hash_id, room_code: exam_room.room_code, answers: { "1" => "A" } }
    assert_redirected_to login_path
    assert_match(/sign in to take the exam/i, flash[:alert].to_s)
  end
  ...
  test "get show without login redirects to login" do
    ...
    get exam_path(exam_session.hash_id, room_code: exam_room.room_code)
    assert_redirected_to login_path
    assert_equal "/exams/#{exam_session.hash_id}?room_code=#{exam_room.room_code}", session[:return_to]
  end
